...
const UnseenChatToast: FC<UnseenChatToastProps> = ({ t, senderId, sessionId, senderImg, senderName, senderMessage }) => {
    return <div className={cn('max-w-md w-full bg-white shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5', {
        ...
        <div className='flex border-l border-gray-200'>
            <button
                className='w-full border border-transparent rounded-none rounded-r-lg p-4 flex items-center justify-center text-sm font-medium text-[#72cab7] hover:text-[#ade6da] focus:outline-none focus:ring-2 focus:ring-[#ade6da]'
                onClick={() => toast.dismiss(t.id)}
            >
                Close
            </button>
